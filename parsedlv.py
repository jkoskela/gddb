...
class Atom():
    ...
    def __init__(self, predicate, terms):
        ...
        for i, e in enumerate(self.terms_list):    #Replace anonymous variables
            if e == '_':
...
class Clause:
    def __init__(self, head, body, index):
        ...
        for atom in self.body:
            self.aux_terms = self.aux_terms.union(set(atom.terms_list)) #Add all terms in the clause to the set
        for i, elem in enumerate(self.aux_terms):
            self.term_map[elem] = i                                     #Map each term to a position in the argument to the aux atom
